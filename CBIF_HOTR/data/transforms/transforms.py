...
from CBIF_HOTR.util.box_ops import box_xyxy_to_cxcywh
from CBIF_HOTR.util.misc import interpolate
...
def resize(image, target, size, max_size=None):
    ...
    if "human_boxes" in target:
        ...
        scaled_human_boxes = human_boxes * torch.as_tensor([ratio_width, ratio_height, ratio_width, ratio_height])
        target["human_boxes"] = scaled_human_boxes
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        if "human_boxes" in target:
            human_boxes = target["human_boxes"]
            human_boxes = box_xyxy_to_cxcywh(human_boxes)
            human_boxes = human_boxes / torch.tensor([w, h, w, h], dtype=torch.float32)
            target["human_boxes"] = human_boxes
