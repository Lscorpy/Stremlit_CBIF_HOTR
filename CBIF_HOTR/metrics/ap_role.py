...
class APRole(object):
    ...
    def add_data(self, h_box, o_box, score, i_box, i_act, p_box, p_act):
        ...
        n_act_pred = score.shape[0]
        n_act_gt   = p_act.shape[1]
        assert n_act_pred == len(self.act_name), (
            f"APRole: predicted score has {n_act_pred} action columns but "
            f"act_name has {len(self.act_name)} entries — vocab length mismatch."
        )
        assert n_act_gt == len(self.act_name), (
            f"APRole: GT pair_act has {n_act_gt} action columns but "
            f"act_name has {len(self.act_name)} entries — vocab length mismatch "
            f"(check that the dataset's action column order matches the model's "
            f"action_idx / valid_ids ordering)."
        )
        ...
        if h_box.shape[0] == 0 or i_box.shape[0] == 0: 
            print(f"Warning: No GT found for an image. Skipping assignment.")
            return # Skip if no predictions OR no ground truth boxes
